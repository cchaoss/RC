...
//pBuffer:数据指针
//NumToWrite:半字数(16位)(就是要写入的16位数据的个数)
...
//从Flash读出参数
void Read_ADDR_Channel(void)
{
	STMFLASH_Read(TABLE_ADDRESS,&flag.ADDR_Channel[0],1);
	if(flag.ADDR_Channel[0] == 0x1234) {
		STMFLASH_Read(TABLE_ADDRESS,flag.ADDR_Channel,sizeof(flag.ADDR_Channel));
		flag.TX_Addr[0] = 0x2a;//
		...
		flag.TX_Channel[0] = flag.ADDR_Channel[3]>>8;
		flag.TX_Channel[1] = flag.ADDR_Channel[3];
		flag.TX_Channel[2] = flag.ADDR_Channel[4]>>8;
		flag.TX_Channel[3] = flag.ADDR_Channel[4];
	}
